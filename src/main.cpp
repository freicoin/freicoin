                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (!tx.CheckInputs(state, view, 0, true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC | SCRIPT_VERIFY_DERSIG))
        {
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    std::pair<int64, int64> dAdjustmentFactor;
    ...
    if (fUseFilter) {
        ...
        int64 vFilteredTime = 0;
        for (idx=0; idx<WINDOW; ++idx)
            vFilteredTime += (int64)kFilterCoeff[idx] * (int64)vTimeDelta[idx];
        ...
        if (vFilteredTime < 597105209444LL) {
            ...
            dAdjustmentFactor.first = ((int64)(kGain.first + kGain.second) * nTargetSpacing) << 31;
            dAdjustmentFactor.first -= (int64)kGain.first * vFilteredTime;
            dAdjustmentFactor.second = ((int64)kGain.second * nTargetSpacing) << 31;
        }
    } else {
...
int64 GetTimeAdjustedValue(int64 value_in, int relative_depth)
{
    const int sign = (value_in > 0) - (value_in < 0);
    const uint64_t value = std::abs(value_in);
    ...
    return sign * static_cast<int64>(sum);
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, int nPerInputAdjustment, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        if (pvChecks)
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            nInput = GetPresentValue(coins, coins.vout[prevout.n], nRefHeight) + nPerInputAdjustment;
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].nRefHeight != pindex->nHeight)
        return state.DoS(100, error("ConnectBlock() : coinbase refheight != block height"));
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            CValidationState state;
            if (!tx.CheckInputs(state, view, 0, true, SCRIPT_VERIFY_P2SH))
                continue;
